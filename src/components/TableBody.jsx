      ...
      {taskToDisplay.map((task, taskIndex) => (
        <>
          <tr key={task.taskDescription}>
            ...
            <td key={`${taskIndex}102${task.taskDescription}`} rowSpan={task.taskUpdate.length + 1} className={`pr-3 pl-3 ${defaultCalsses}`}>
          ...
          {task.taskUpdate.map((update, updateIndex) => (
            <tr key={`${taskIndex}${updateIndex}8`}>
              ...
              <td key={`${taskIndex}${updateIndex}91`} className={`pr-2 pl-2 ${defaultCalsses}`}>
                {update[1]}
              </td>
              <td key={`${taskIndex}${updateIndex}92`} className={`pr-2 pl-2 ${defaultCalsses}`}>
                {update[2]}
              </td>
            </tr>
          ))}
